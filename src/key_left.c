...
/*   Updated: 2022/07/01 19:13:16 by Yago_42          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  if(ft_strncmp(map->map_textures[nxt_y][nxt_x]->path, OBSTACLE_PATH, 69))
    {
      ...
      {
        ft_end_game_left(map, nxt_x, nxt_y);
      }
    }
}
...
void  ft_mov_left(t_map_data *map, int nxt_x, int nxt_y)
{
    ...
    mlx_image_to_window(map->mlx, mlx_texture_to_image(map->mlx, mlx_load_png(PJL_PATH)), nxt_x * 64 , nxt_y * 64);
    mlx_delete_image(map->mlx, map->map_textures[nxt_y][nxt_x + 1]->img);
    mlx_image_to_window(map->mlx, mlx_texture_to_image(map->mlx, mlx_load_png(LAND_PATH)), (nxt_x + 1) * 64 , nxt_y * 64);
    ft_map_moves(map);
